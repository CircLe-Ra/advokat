...
<x-partials.sidebar  :back="route('leader.active.case')" :id-detail="$this->case?->id" menu="leader-active-case" active="Penanganan Kasus / Jadwal Pertemuan / {{ $this->case?->title }}">
    <x-slot:information>
        ...
        <div class="flex flex-col  dark:border-zinc-700 dark:bg-zinc-800 flex-shrink-0">
            <div
                ...
                <div class="h-20 w-20 rounded-full border border-zinc-200 dark:border-zinc-700 overflow-hidden">
                    <flux:avatar size="xl" class="size-full " :name="$this->case->lawyer?->user->name" :initials="$this->case->lawyer?->user->initials()"/>
                </div>
                            ...
                            <flux:menu>
                                <flux:menu.group heading="Kasus">
                                    <flux:menu.item icon:variant="micro" icon:trailing="arrow-up-right" icon="file-plus-2"
                                                    href="{{ route('leader.active.case.page', ['case' => $schedule->id, 'status' => 'meeting-result']) }}" wire:navigate>
                                        Hasil Pertemuan
                                    </flux:menu.item>
                                    <flux:menu.separator/>
                                    <flux:menu.item icon:variant="micro" icon:trailing="arrow-up-right" icon="photo"
                                                    href="{{ route('leader.active.case.page', ['case' => $schedule->id, 'status' => 'meeting-documentation']) }}" wire:navigate>
                                       Dokumentasi
                                    </flux:menu.item>
                                </flux:menu.group>
                            </flux:menu>
