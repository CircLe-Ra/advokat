...
<x-partials.sidebar :id-detail="$this->meeting->legalCase?->id" menu="leader-active-case" :back="route('leader.active.case.page', ['case' => $this->meeting->legal_case_id, 'status' => 'schedule'], absolute: false)"
    ...
    <x-slot:information>
        @if($this->meeting->legalCase?->status !== 'rejected')
            <div class="flex flex-col  dark:border-zinc-700 dark:bg-zinc-800 flex-shrink-0">
                <div
                    ...
                    <div class="h-20 w-20 rounded-full border border-zinc-200 dark:border-zinc-700 overflow-hidden">
                        <flux:avatar size="xl" class="size-full " :name="$this->meeting->legalCase?->lawyer?->user->name" :initials="$this->meeting->legalCase?->lawyer?->user->initials()"/>
                    </div>
