...
<div>
    ...
    <x-table label="Kasus" sub-label="Daftar kasus yang anda tangani."
             thead="#, No. Kasus, No. Perkara, Nama, Jenis, Tanggal Pengajuan, Status">
        <x-slot name="filter">
        ...
        @if($this->cases->count() > 0)
            @foreach($this->cases as $case)
                <tr class="bg-white border-b dark:bg-zinc-800 dark:border-zinc-700 border-zinc-200 ">
                    ...
                    <td>
                        <flux:button wire:navigate variant="outline" icon:trailing="arrow-right" size="sm" class="cursor-pointer dark:bg-zinc-800 dark:hover:bg-zinc-800" href="{{ route('lawyer.case.page', ['case' => $case->id, 'status' => 'schedule']) }}">
                            Detail
                        </flux:button>
                    </td>
                </tr>
