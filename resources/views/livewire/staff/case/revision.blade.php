...
new class extends Component {
    ...
    {
        return LegalCase::where('status', 'revision')
            ->where(function ($query) {
                $query->where('number', 'like', '%' . $this->search . '%')
                    ->orWhere('title', 'like', '%' . $this->search . '%')
                    ->orWhere('summary', 'like', '%' . $this->search . '%');
            })->orWhere('status', 'revised')->latest()->paginate($this->show, pageName: 'revision-page');
    }
    ...
    public function showProfil($id): void
    {
                            ...
                            <flux:menu>
                                <flux:menu.group heading="Kasus">
                                    <flux:menu.item wire:navigate
                                                    href="{{ route('staff.case.detail-case', ['id' => $case->id, 'status' => 'revision']) }}"
        ...
        <div class="space-y-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2">
                ...
                <div class="border border-zinc-200 p-2">
                    ...
                    <img src="{{ asset('storage/' . $this->client?->identity_image) }}" alt="Foto Identitas"
                         class="w-40 rounded border">
                </div>
                <div class="border border-zinc-200 p-2">
                    <flux:label>Foto Klien</flux:label>
                    <img src="{{ asset('storage/' . $this->client?->client_image ?? '') }}" alt="Foto Klien"
                         class="w-40 rounded border">
                </div>
                <div class="border border-zinc-200 p-2">
